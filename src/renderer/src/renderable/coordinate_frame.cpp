...
void CoordinateFrame::AllocateGpuResources() {
  ...
  if (!vertex_shader.Compile()) {
    std::cerr << "ERROR::COORDINATE_FRAME::VERTEX_SHADER_COMPILATION_FAILED" << std::endl;
    throw std::runtime_error("Vertex shader compilation failed");
  }
  
  if (!fragment_shader.Compile()) {
    std::cerr << "ERROR::COORDINATE_FRAME::FRAGMENT_SHADER_COMPILATION_FAILED" << std::endl;
...
void CoordinateFrame::GenerateAxes() {
  ...
  if (is_2d_mode_) {
    // In 2D mode, we want X and Y axes on the XY plane (Z=0)
    ...
    // Y-axis line (green) - In 2D mode, this is along the Y axis (Z in OpenGL)
