 ...
 * @date 2025-03-17
 * @brief Handles transformations between OpenGL coordinate system (Y-up) and
 * standard coordinate system (Z-up)
 *
 ...
 *
 * This class provides utilities to transform between the standard coordinate
 * system (Z-up, Y-forward, X-right) and the OpenGL coordinate system (Y-up,
 * Z-forward, X-right).
 *
...
class CoordinateTransformer {
 public:
  ...
  static glm::mat4 GetStandardToOpenGLTransform() {
    // Rotate -90 degrees around X axis: Z-up becomes Y-up, Y-forward becomes
    // Z-forward
  ...
  static glm::mat4 GetOpenGLToStandardTransform() {
    // Rotate 90 degrees around X axis: Y-up becomes Z-up, Z-forward becomes
    // Y-forward
    return glm::rotate(glm::mat4(1.0f), glm::radians(90.0f),
                       glm::vec3(1.0f, 0.0f, 0.0f));
  }
  ...
  /**
   * @brief Transform a direction vector from standard to OpenGL coordinate
   * system
   *
  ...
  static glm::vec3 TransformDirectionToOpenGL(const glm::vec3& direction) {
    return glm::vec3(GetStandardToOpenGLTransform() *
                     glm::vec4(direction, 0.0f));
  }

  /**
   * @brief Transform a direction vector from OpenGL to standard coordinate
   * system
   *
  ...
  static glm::vec3 TransformDirectionToStandard(const glm::vec3& direction) {
    return glm::vec3(GetOpenGLToStandardTransform() *
                     glm::vec4(direction, 0.0f));
  }
};
