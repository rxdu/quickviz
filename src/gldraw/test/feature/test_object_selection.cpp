    ...
    if (!IsEmpty(last_selection_)) {
      if (std::holds_alternative<PointSelection>(last_selection_)) {
        ...
        if (point_selection.cloud_name == "grid_points") {
          ImGui::TextColored(ImVec4(0.7f, 0.7f, 0.7f, 1.0f), "    (Grid Pattern)");
...
class SelectionDemoPanel : public GlScenePanel {
 public:
  SelectionDemoPanel(const std::string& name, SelectionInfoPanel* info_panel)
 ...
 private:
  ...
  SelectionMode selection_mode_ = SelectionMode::kHybrid;  // Default to hybrid mode
};

int main() {
  std::cout << "=== SelectionManager API Test ==="
            << std::endl;
  std::cout << "This test demonstrates the new unified selection API for:"
            << std::endl;
  std::cout << "  - Point selection from point clouds"
            << std::endl;
  std::cout << "  - Object selection (spheres)"
            << std::endl;
  std::cout << "  - Multi-selection with Ctrl+Shift+Click"
            << std::endl;
  std::cout << "  - Toggle selection with Ctrl+Alt+Click"
            << std::endl;
  std::cout << std::endl;
  ...
  try {
    ...
    auto scene_panel =
        std::make_shared<SelectionDemoPanel>("3D Scene", info_panel.get());
    ...
    std::cout << "✓ Grid point cloud created with " << points.size() << " points" << std::endl;
    ...
    std::cout << "✓ Spiral point cloud created with " << spiral_points.size() << " points" << std::endl;
    ...
    std::cout << "  - Colors: Bottom layer (RED), Middle layer (GREEN), Top "
                 "layer (BLUE)"
              << std::endl;
    std::cout << "  - Grid point cloud (100 points) at z=2.0 for point selection testing"
              << std::endl;
    std::cout << "  - Spiral point cloud (50 points) at z=3.0-5.0 with rainbow colors"
              << std::endl;
    ...
    std::cout << "\n✓ Selection test ready!"
              << std::endl;
    std::cout << "\n=== Scene Contents ==="
              << std::endl;
    std::cout << "- Grid Point Cloud: 'grid_points' with 100 points at z=2.0 (green/cyan grid)"
              << std::endl;
    std::cout << "- Spiral Point Cloud: 'spiral_points' with 50 points at z=3.0-5.0 (rainbow spiral)"
              << std::endl;
    std::cout << "- Spheres: 9 spheres (sphere_00 to sphere_08) at z=0.0 (red color)"
              << std::endl;
