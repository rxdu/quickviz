...
PointCloud::~PointCloud() {
    DeinitGraphicsResources();
}

void PointCloud::InitGraphicsResources() {
    try {
...
void PointCloud::DeinitGraphicsResources() {
...
void PointCloud::SetScalarField(const std::vector<float>& scalars, float min_val, float max_val) {
    ...
    std::cout << "Set scalar field with range [" << min_val << ", " << max_val << "]" << std::endl;
}
...
void PointCloud::OnDraw(const glm::mat4& projection, const glm::mat4& view) {
    ...
    for (int i = 0; i < std::min(3, static_cast<int>(points_.size())); i++) {
        std::cout << "  Point " << i << ": " << points_[i].x << ", " 
                  << points_[i].y << ", " << points_[i].z << std::endl;
    }
    ...
    if (needs_update_) {
        ...
        needs_update_ = false;
        std::cout << "Updated VBO data with " << points_.size() << " points" << std::endl;
    }
    ...
    std::cout << "Drawing " << points_.size() << " points with size 50.0" << std::endl;
