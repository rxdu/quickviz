    ...
    void SetUp(const ::benchmark::State& state) override {
        ...
        if (display_available_) {
            try {
                ...
                scene_manager_ = std::make_shared<GlScenePanel>("BenchmarkScene");
...
static void BM_SceneObjectCreation(benchmark::State& state) {
    ...
    for (auto _ : state) {
        std::vector<std::shared_ptr<GlScenePanel>> scenes;
        scenes.reserve(object_count);
        
        for (int i = 0; i < object_count; ++i) {
            auto scene = std::make_shared<GlScenePanel>("Scene" + std::to_string(i));
